...
__global__ void block_matching(uint8_t* ref_frame, uint8_t* curr_frame, int* mv, int width, int height, int BLK_SIZE, int srch_range)
{
    ...
    __shared__ int16_t mv_list[2048];
    __shared__ uint8_t curr_frame_shared[1024];
    // __shared__ uint8_t ref_frame_shared[1024];
    ...
    curr_frame_shared[threadIdx.y*BLK_SIZE + threadIdx.x] = curr_frame[macro_y*width+macro_x + threadIdx.y*width+threadIdx.x];
    __syncthreads(); 
        ...
        for(int sub_macro_x=macro_x-srch_range; sub_macro_x<macro_x+srch_range; sub_macro_x+=BLK_SIZE)
        {
            ...
            for(int i=0; i<BLK_SIZE; i++)
            {
                for(int j=0; j<BLK_SIZE; j++)
                {
                    ...
                    SAD += abs((int)(int)ref_frame[ref_y*width+ref_x + i*width+j] - (int)curr_frame_shared[i*BLK_SIZE+j]);
                }
            }

            // if(blockIdx.x ==12 && blockIdx.y==4)
            //     printf("macro: %d %d %d %d %d\n", macro_y, macro_x, ref_y, ref_x, SAD);

            SAD_list[threadIdx.y*2*sub_srch_range + threadIdx.x] = SAD;
            mv_list[threadIdx.y*2*sub_srch_range*2 + threadIdx.x*2] = (int16_t)ref_y - macro_y;
            mv_list[threadIdx.y*2*sub_srch_range*2 + threadIdx.x*2+1] = (int16_t)ref_x - macro_x;
            __syncthreads();              
            ...
            {
                min_SAD = SAD_list[0];
                best_y = (int)mv_list[SAD_indexs[0]*2];
                best_x = (int)mv_list[SAD_indexs[0]*2 + 1];
            }
        }
    }
